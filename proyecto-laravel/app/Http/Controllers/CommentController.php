...
class CommentController extends Controller
{
    ...
    public function delete($id){
        ...
        if($user&&($comment->user->id==$user->id||$comment->image->id==$user->id)){
            $comment->delete();
            return redirect(route('image.detail',['id'=>$comment->image->id]))->with(['message'=>'Comentario borrado correctamente']);
        }else{
